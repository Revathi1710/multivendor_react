...
class ProductSlider extends Component {
  ...
  fetchSlides = async () => {
    try {
      const response = await axios.get("http://localhost:5000/getProductsHome");
        ...
        {!loading && !error && (
          <Slider {...settings}>
            {slides.map((slide, index) => (
              <div key={index}>
                <ProductCardTwo imgSrc={`http://localhost:5000/${slide.image.replace('\\', '/')}`} name={slide.name} description={slide.description} />
              </div>
            ))}
          </Slider>
        )}
