...
const AllSlider = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/AllSliderAdmin', {
  ...
  const handleUpdate = (productId) => {
    window.location.href = `/SuperAdmin/UpdateSlider/${productId}`;
  };

  const handleDelete = (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      fetch('http://localhost:5000/deleteSlider', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div>
      ...
      <div className="" style={{ marginLeft: '250px' }}>
        <div className="title"><h2>All Slider Images</h2></div>
        {message && <p>{message}</p>}
        {products.length > 0 ? (
